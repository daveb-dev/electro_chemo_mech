...
ComputeTlNeoHookeanStress::computeQpStress()
{
    ...
    RankTwoTensor C = _deformation_gradient[_qp]*_deformation_gradient[_qp].transpose();
    RankTwoTensor B = _deformation_gradient[_qp].transpose()*_deformation_gradient[_qp];
    RankTwoTensor Cinv = C.inverse();
    // Material Portion of the Jacobian matrix 
    _stress[_qp] = lambda0*logJ*II + mu0*(B-II);
    _Jacobian_mult[_qp] = lambda0*Cinv.outerProduct(Cinv) + mu*(Cinv.mixedProductIkJl(Cinv) + Cinv.mixedProductIlJk(Cinv.transpose()));
}
