...
/* 
 * File:   StressDivergenceConcentrationTensors.C
 * Author: srinath
 * 
 * Created on September 10, 2018, 3:02 PM
 */

#include "StressDivergenceConcentrationTensors.h"
...
registerMooseObject("TensorMechanicsApp", StressDivergenceConcentrationTensors);

template <>
InputParameters
validParams<StressDivergenceConcentrationTensors>()
{
  ...
  params.set<bool> ("use_deformed_mesh") = false;
  return params;
}

StressDivergenceConcentrationTensors::StressDivergenceConcentrationTensors(const InputParameters & parameters)
...
void
StressDivergenceConcentrationTensors::initialSetup()
{
...
void
StressDivergenceConcentrationTensors::computeResidual()
{
...
Real
StressDivergenceConcentrationTensors::computeQpResidual()
{
    Real J = (*_deformation_gradient)[_qp].det();
    RankTwoTensor k_stress = _stress[_qp]*J;
...
void
StressDivergenceConcentrationTensors::computeJacobian()
{
  if (_use_finite_deform_jacobian)
  {
...
void
StressDivergenceConcentrationTensors::computeOffDiagJacobian(MooseVariableFEBase & jvar)
{
  if (_use_finite_deform_jacobian)
  {
    _finite_deform_Jacobian_mult.resize(_qrule->n_points());
...
Real
StressDivergenceConcentrationTensors::computeQpJacobian()
{
  if (_use_finite_deform_jacobian)
...
Real
StressDivergenceConcentrationTensors::computeQpOffDiagJacobian(unsigned int jvar)
{
  ...
  // off-diagonal Jacobian with respect to a coupled concentration variable
  if (_conc_coupled && jvar == _conc_var) 
        return -((_Jacobian_mult[_qp] * (*_deigenstrain_dC)[_qp]) *
             _grad_test[_i][_qp])(_component)*_phi[_j][_qp];
...
void
StressDivergenceConcentrationTensors::computeFiniteDeformJacobian()
{
...
void
StressDivergenceConcentrationTensors::computeAverageGradientTest()
{
...
void
StressDivergenceConcentrationTensors::computeAverageGradientPhi()
{
...
void 
StressDivergenceConcentrationTensors::computeAverageVolumeChange()
{
    
    _avg_vol_change = 0.0;
    for (_qp = 0; _qp < _qrule->n_points(); ++_qp)
        _avg_vol_change += (*_deformation_gradient)[_qp].det() * _JxW[_qp] * _coord[_qp];

      _avg_vol_change /= _current_elem_volume;
}
